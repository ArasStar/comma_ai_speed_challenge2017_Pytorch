...
def train(valid=False, test=False ,save_model=False):
    print("hoop")
    #    MODEL
    num_epoch = 300 #100 #90
    batch_size= 4
    ...
    train_set = CustomDataset("small.csv",ROOT)
    valid_set = CustomDataset("valid.csv",ROOT)
    ...
    plotloss = []
    running_loss=0.0
    n_iter = len(train_set)/batch_size
    model.train()
    ...
    for epoch in range(num_epoch):
        for i, (images, speeds) in enumerate(trainloader):
            ...
            if i%10==9:
                print(f'epoch:{epoch+1}, {i+1}/{n_iter} loss: {running_loss/10}')
                running_loss = 0.0
